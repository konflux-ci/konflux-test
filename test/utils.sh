...
# This function wraps get_image_manifests() and returns a bash associative array {arch:digest} for the given image_url.
get_image_manifests_array() {
  local image_url=""

  #Usage information
  get_image_manifests_array_usage()
  {
    echo "
  get_image_manifests_array -i IMAGE_URL
  " >&2
    exit 2
  }
  ...
  if [ -z "${image_url}" ]; then
    echo "get_image_manifests_array: Missing keyword parameter (-i IMAGE_URL)" >&2
    exit 2
  fi

  local image_manifests
  image_manifests=$(get_image_manifests -i "${image_url}")

  declare -A manifests_array
  while IFS="=" read -r key value
  do
      manifests_array[$key]="$value"
  done < <(echo "${image_manifests}" | jq -r 'to_entries | map("\(.key)=\(.value)") | .[]')
  echo "${manifests_array[@]@K}"
}
...
# base image). If the index image is provided with a digest it will use that instead of
# deriving the matching index image.
...
extract_differential_fbc_metadata() {
  ...
  local index_digest index_tag index_url
