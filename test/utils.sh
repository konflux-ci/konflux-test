...
get_ocp_version_from_fbc_fragment() {
  ...
  # Attempt to retrieve the ocp version list from the fragment labels
  ...
  if [[ -n "$ocp_version_list_raw" ]]; then
      ocp_version_list=$(
        echo "$ocp_version_list_raw" \
          | cut -d'=' -f2- \
          | jq -r '.' \
      )

    # Fail if the label content is not an array or an empty array
    if ! printf '%s\n' "$ocp_version_list" | jq -e 'type=="array" and length>0' >/dev/null 2>&1; then
      echo "get_ocp_version_from_fbc_fragment: Label \"com.redhat.fbc.openshift.version\"
      must contain a non-empty JSON array, got: $ocp_version_list" >&2
      exit 2
    fi
