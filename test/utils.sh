...
get_image_repository() {
  ...
  parse_image_url "$image_url" | jq -jr '.repository'
}
...
get_base_image() {
  ...
  image_manifest=$(parse_image_url "$image" | jq -ej ".repository + \"@$image_manifest_digest\"")
  ...
  if [ -n "$base_image_digest" ]; then
    ...
    base_image="$(get_image_repository_tag "$base_image")@$base_image_digest"
  fi
  echo -n "$base_image"
}
...
get_ocp_version_from_fbc_fragment() {
  ...
  local base_image
  base_image=$(get_image_repository_tag "$(get_base_image "$FBC_FRAGMENT")")
...
# This function can be used to get the catalog image matching a FBC fragment.
get_matching_fbc_catalog_image() {
  local FBC_FRAGMENT=""
  local INDEX_IMAGE="$DEFAULT_INDEX_IMAGE"

  get_matching_fbc_catalog_image_usage()
  {
    echo "
  get_matching_fbc_catalog_image -i FBC_FRAGMENT -b INDEX_IMAGE
  " >&2
    exit 2
  }
  ...
  if [ -z "$FBC_FRAGMENT" ]; then
    echo "get_matching_fbc_catalog_image: missing keyword parameter (-i FBC_FRAGMENT)" >&2
    exit 2
  fi

  if [ -z "$INDEX_IMAGE" ]; then
    echo "get_matching_fbc_catalog_image: missing keyword parameter (-b INDEX_IMAGE)" >&2
    exit 2
  fi

  # If the index image is provided and has a tag, remove it.
  # The target ocp version is determined from the fragment
  INDEX_IMAGE=$(parse_image_url "$INDEX_IMAGE" | jq -jr '.repository')

  #Get target ocp version from the fragment
  local ocp_version
  if ! ocp_version=$(get_ocp_version_from_fbc_fragment "$FBC_FRAGMENT"); then
    echo "get_matching_fbc_catalog_image: could not get ocp version for the fragment" >&2
    exit 1
  fi
...
render_opm() {
  ...
  mkdir -p "$CACHE_DIR"
  CACHE_SUBDIR=$(parse_image_url "$RENDER_TARGET" | jq -jr '.repository + if .tag != "" then "/" + .tag else "" end + if .digest != "" then "/" + .digest else "" end')
...
extract_differential_fbc_metadata() {
  ...
  if ! tagged_index=$(get_matching_fbc_catalog_image -i "$FBC_FRAGMENT" -b "$INDEX_IMAGE"); then
