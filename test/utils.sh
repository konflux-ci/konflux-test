...
upload_file() {
  ...
  if ! raw_inspect_output=$(skopeo inspect --no-tags --raw docker://"${OCI_SUBJECT}"); then
    echo "Failed to inspect ${OCI_SUBJECT}" >&2
    exit 2
  fi
  if ! DIGEST=$(echo "${raw_inspect_output}" | jq .Digest); then
    echo "Failed to identify digest of ${OCI_SUBJECT}"
    exit 2
  fi

  PATH=$(echo "$OCI_SUBJECT" | awk -F ':' '{ print $1 }')

  oras push  "${PATH}:sha256-${DIGEST}.${SUFFIX} ${UPLOAD_FILE}:${MEDIA_TYPE}"
