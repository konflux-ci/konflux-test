...
@test "Get Unreleased FBC related images: valid-fragment-fbc-success" {
    run get_unreleased_fbc_related_images -i valid-fragment-fbc-success
    EXPECTED_RESPONSE=$(echo "registry.redhat.io/foo/bar@sha256:my-bar-sha registry.redhat.io/foo/baz@sha256:my-sha"  | tr ' ' '\n')
    [[ "${EXPECTED_RESPONSE}" = "${output}" && "$status" -eq 0 ]]
}

@test "Get Unreleased FBC related images: valid-fragment-fbc-success and index with tag" {
    run get_unreleased_fbc_related_images -i valid-fragment-fbc-success -b registry.redhat.io/redhat/redhat-operator-index:v4.15@randomsha256
    EXPECTED_RESPONSE=$(echo "registry.redhat.io/foo/bar@sha256:my-bar-sha"  | tr ' ' '\n')
    [[ "${EXPECTED_RESPONSE}" = "${output}" && "$status" -eq 0 ]]
}

@test "Get Unreleased FBC related images: valid-fragment-fbc-success-2 and custom index" {
    run get_unreleased_fbc_related_images -i valid-fragment-fbc-success-2 -b registry.io/random-index:v4.20
    EXPECTED_RESPONSE=""
    [[ "${EXPECTED_RESPONSE}" = "${output}" && "$status" -eq 0 ]]
}
